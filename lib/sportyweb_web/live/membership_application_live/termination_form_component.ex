defmodule SportywebWeb.MembershipApplicationLive.TerminationFormComponent do
  ...
  def handle_event("save", %{"termination_form" => params}, socket) do
    ...
    if Enum.empty?(changeset.errors) do
        send_update(SportywebWeb.MembershipApplicationLive, [id: "application-form", value: changeset])
        {:noreply, socket}
      else
        IO.inspect(changeset)
        {:noreply, assign(socket, form: to_form(changeset, action: :validate, as: :termination_form))}

    end
  end

  defp form_changeset(params) do
    types = %{person_last_name: :string, person_first_name_1: :string,
      birthday: :date, email: :string, termination_date: :date}
  ...
  defp validate_input(params) do
    ...
    |> Ecto.Changeset.validate_required([:person_last_name, :person_first_name_1, :birthday, :email, :termination_date])
  end
end
